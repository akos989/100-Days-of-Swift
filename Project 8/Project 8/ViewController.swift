...
//  Project 8
//
//  Created by Ákos Morvai on 2022. 03. 30..
//
...
class ViewController: UIViewController {
    ...
    @objc func submitTapped(_ sender: UIButton) {
        ...
        if let solutionPosition = solutions.firstIndex(of: answerText) {
            ...
            currentAnswer.text = ""
            score += 1
            remainingWordsInLevel -= 1
            
            if remainingWordsInLevel == 0 {
                let ac = UIAlertController(title: "Well done!", message: "Are you ready for the next level?", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "Let's go!", style: .default, handler: levelUp))
                present(ac, animated: true)
            }
        } else {
            let ac = UIAlertController(title: "Wrong answer!", message: "This word is not in the solutions", preferredStyle: .alert)
    ...
    private func loadLevel() {
        ...
        if let levelFileUrl = Bundle.main.url(forResource: "level\(level)", withExtension: ".txt"),
           let levelContents = try? String(contentsOf: levelFileUrl) {
            ...
            lines.shuffle()
            remainingWordsInLevel = lines.count
