...
//  Project23
//
//  Created by Ákos Morvai on 2022. 06. 25..
//
...
enum Constants {
    static let minXEnemyCreationPosition = 64
    static let maxXEnemyCreationPosition = 906
    static let yEnemyCreationPosition = -128
    
    static let minAngularVelocityValue = -3
    static let maxAngularVelocityValue = 3
}
...
class GameScene: SKScene {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for case let node as SKSpriteNode in nodesAtPoint {
            if node.name == "enemy" {
                ...
                if let index = enemies.firstIndex(of: bombContainer) {
                    enemies.remove(at: index)
                }
    ...
    override func update(_ currentTime: TimeInterval) {
        if enemies.count > 0 {
            for (index, node) in enemies.enumerated().reversed() {
    ...
    func redrawActiveSlice() {
        ...
        if activeSlicePoints.count > 12 {
            activeSlicePoints.removeLast(activeSlicePoints.count - 12)
        }
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        let randomPosition = CGPoint(x: Int.random(in: Constants.minXEnemyCreationPosition...Constants.maxXEnemyCreationPosition), y: Constants.yEnemyCreationPosition)
