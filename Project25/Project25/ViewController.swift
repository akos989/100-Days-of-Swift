...
//  Project25
//
//  Created by Ákos Morvai on 2022. 07. 08..
//
...
class ViewController:
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItems = [
            ...
            UIBarButtonItem(barButtonSystemItem: .search, target: self, action: #selector(showConnectedDevices))
        ]
    ...
    func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        switch state {
            ...
            case .notConnected:
                ...
                let ac = UIAlertController(title: "\(peerID.displayName) unconnected", message: nil, preferredStyle: .alert)
    ...
    func session(_ session: MCSession, didReceive data: Data, fromPeer peerID: MCPeerID) {
        DispatchQueue.main.async { [weak self] in
            if let image = UIImage(data: data) {
                ...
                let ac = UIAlertController(title: "Message received!", message: "Message body: \(message)", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "OK", style: .default))
    ...
    @objc func writeMessage() {
        let ac = UIAlertController(title: "Message", message: "Write your message you wanna send!", preferredStyle: .alert)
        ac.addTextField()
            ...
            if let text = ac?.textFields?[0].text,
               let mcSession = self?.mcSession {
                if mcSession.connectedPeers.count > 0 {
                    ...
                    do {
                        try mcSession.send(dataString, toPeers: mcSession.connectedPeers, with: .reliable)
                    } catch {
                        let ac = UIAlertController(title: "Send error", message: error.localizedDescription, preferredStyle: .alert)
                        ac.addAction(UIAlertAction(title: "OK", style: .default))
                        self?.present(ac, animated: true)
    ...
    @objc func showConnectedDevices() {
        if let mcSession = mcSession {
            let connectionNames = mcSession.connectedPeers.map { mCPeerID in
                return "\(mCPeerID.displayName), "
            }
            let ac = UIAlertController(title: "Connected devices", message: "\(connectionNames)", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "OK", style: .default))
            present(ac, animated: true)
        }
    }
